...
//    attention = digitalRead(_irq);
    attention = HAL_GPIO_ReadPin(TOUCH_INT_GPIO_Port, TOUCH_INT_Pin);
...
//    Wire.endTransmission();
	...
	buf[0] = reg;
	buf[1] = val;
    i2c2_status = HAL_I2C_Master_Transmit(&hi2c1, FT6236_ADDR, buf, 2, 100);
...
//    uint8_t retVal = Wire.endTransmission();
	...
	uint8_t retVal = 0;
	i2c2_status = HAL_I2C_Master_Transmit(&hi2c1, FT6236_ADDR, &reg, 1, 100);
	if(i2c2_status == HAL_OK) {
		i2c2_status = HAL_I2C_Master_Receive(&hi2c1, FT6236_ADDR, &retVal, 1, 100);
	}
	if(i2c2_status != HAL_OK) {
		HAL_Delay(1);
	}
...
//    }
	...
	i2c2_status = HAL_I2C_Master_Transmit(&hi2c1, FT6236_ADDR, &regAddr, 1, 100);
	if(i2c2_status == HAL_OK) {
		i2c2_status = HAL_I2C_Master_Receive(&hi2c1, FT6236_ADDR, pBuf, len, 100);
	}
	if(i2c2_status != HAL_OK) {
			HAL_Delay(1);
		}
